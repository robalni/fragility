            ...
            if(target->state == CS_DEAD || target->state == CS_WAITING)
            {
                ...
                if(shownotices >= 2)
                {
                    if(target == &game::player1 && !client::demoplayback)
                    {
                        ...
                        {
                            pushfont("little");
                            ty += draw_textf("Press \fs\fw\f{=show_ui profile 2}\fS to \fs%s\fS loadout", tx, ty, int(FONTW*noticepadx), int(FONTH*noticepady), tr, tg, tb, tf, TEXT_CENTERED, -1, tw, 1, target->loadweap.empty() ? "\fzoyselect" : "change");
                            popfont();
                        }
                        if(m_play(game::gamemode) && m_team(game::gamemode, game::mutators))
                        {
                            pushfont("little");
                            ty += draw_textf("Press \fs\fw\f{=show_ui team}\fS to change teams", tx, ty, int(FONTW*noticepadx), int(FONTH*noticepady), tr, tg, tb, tf, TEXT_CENTERED, -1, tw, 1);
                            popfont();
                        }
                    }
                }
