...
namespace physics
{
    ...
    VAR(IDF_PERSIST, dashstyle, 0, 1, 2); // 0 = only with impulse, 1 = double tap, 2 = double tap only when crouching
    ...
    #define imov(name,v,u,d,s,os) \
        ...
        { \
            ...
            if(down) \
            { \
                if(allowimpulse(&game::player1, A_A_DASH) && (dashstyle == 1 || (dashstyle == 2 && game::player1.action[AC_CROUCH])) && impulseaction&2 && last##v && lastdir##v && dir == lastdir##v && lastmillis-last##v < PHYSMILLIS) \
                { \
