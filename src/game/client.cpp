...
namespace client
{
    ...
    void vote(gameent *d, const char *text, int mode, int muts)
    {
        ...
        if(showmapvotes >= (!gs_playing(game::gamestate) ? 2 : 1) && !isignored(d->clientnum))
            conoutft(CON_EVENT, "%s suggests: \fs\fy%s\fS on \fs\fo%s\fS, press \f{=show_ui maps 2} to vote", game::colourname(d), server::gamename(mode, muts), m->map);
    }
    ...
    void c2sinfo(bool force) // send update to the server
    {
        ...
        if (totalmillis - lastupdate < UPDATERATE && !force) return;    // don't update faster than 125fps
